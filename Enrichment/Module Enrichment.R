...
# both blue and turquoise on one side
...
# blue is positive and turquoise is negative, use Powerpoint to remove the negative symbol
df2 = df
df2$`Protein Ratio` = ifelse(df2$Module == "Turquoise",(-1)*df2$`Protein Ratio`,df2$`Protein Ratio`)
df2$Module = sub("Turquoise","Orange",df2$Module)

tiff("enrich_module2.tiff",res = 300, width = 19, height = 10, units = "in") # this ratio is ideal for landscape into powerpoint
ggplot(data = df2,aes(x = reorder(Description,`Protein Ratio`),y = `Protein Ratio`))+
  geom_bar(mapping = aes(x = reorder(Description,`Protein Ratio`),y = `Protein Ratio`,fill = Module,group = Module),stat = "identity",position = position_dodge()) +
  facet_wrap(.~factor(Type,levels = c("GO BP","GO CC","GO MF","KEGG Enrichment")),nrow = 4,scale = "free",strip.position = "left") +
  scale_fill_identity(guide = "legend", labels = c("Module 1","Module 2")) +
  coord_flip() + 
  #geom_text(aes(label = Description,vjust = 0.5, hjust = ifelse(Module == "Turquoise", 0, 0))) # at the beginning of the bar +
  geom_text(aes(label = Description,vjust = 0.5, hjust = ifelse(Module == "Orange", 1.05, - 0.05)),size = 5) + # at the end of the bar  
  labs(x = "Enrichment Funciton", y = "Protein Ratio") +
  theme_light() +
  ylim(-0.3,0.53) +
  force_panelsizes(rows = c(1,0.5,0.5,3)) +
  theme(legend.position = "bottom", legend.direction = "horizontal", legend.text = element_text(size = 15),axis.title = element_text(size =15),axis.text.x = element_text(size = 15),axis.text.y = element_blank(), strip.text.y = element_text(size = 15))
dev.off()
