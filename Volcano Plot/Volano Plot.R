setwd("G:/My Drive/Perchlorate Project/For Github/To Upload to Github/C34h-Subzero-Perchlorate-Proteomic-Response/NMDS")
...
new_nm = read.csv('C34H_abacus_new_naming.csv',header = T,row.names = 1)

logfold_limit = 0.5
zstat_limit = 2

module_colors = c("blue","orange","brown")
...
Qspec_CW[grepl(paste(module_proteins[["module_turquoise"]]$V1,collapse = "|"),rownames(Qspec_CW)),"WGCNA group"] = "orange"
...
#get new description
test = Inc_sig[,c(1:4,8:11)]
test = merge.data.frame(test,new_nm,by.x = "Protein",by.y = 0, all.x = T)
test$orig = NULL

#merge it again with the original QSPEC
Qspec_CW$new_des = new_nm$des_new[match(Qspec_CW$Protein,rownames(new_nm))]
#Qspec_CW$new_des = test$des_new[match(Qspec_CW$Protein,rownames(new_nm))]


#calculate the percentage of blue module proteins in the significantly increased proteins
Inc_sig %>% group_by(`WGCNA group`) %>% summarise(total_count = n()) 
...
df3[which(abs(df3['LogFoldChange']) >= 0.5 & between (df3$log.p.value, 1.301, 2) & df3["WGCNA group"] == "orange"),"group"] = "Module2_0.05" # between 0.05 and 0.01
df3[which(abs(df3['LogFoldChange']) >= 0.5 & df3$log.p.value > 2 & df3["WGCNA group"] == "orange"),"group"] = "Module2_0.01" # between 0.01 and 

df3[which(abs(df3['LogFoldChange']) >= 0.5 & between (df3$log.p.value, 1.301, 2) & df3["WGCNA group"] == "blue"),"group"] = "Module1_0.05" # between 0.05 and 0.01
df3[which(abs(df3['LogFoldChange']) >= 0.5 & df3$log.p.value > 2 & df3["WGCNA group"] == "blue"),"group"] = "Module1_0.01" # between 0.01 and 
...
cols<- c(NotSignificant="#FFFFFF", Other_0.05 = "#a9a9a9",Other_0.01 = "#656565",Module2_0.05="#FFCC66", Module2_0.01="#FF9900", Module1_0.05 = "#7CB9E8", Module1_0.01 = "#3457D5")
outline<- c(NotSignificant="gray", Other_0.05 = "#555555", Other_0.01 = "#555555" ,Module2_0.05="#CC6633", Module2_0.01="#CC6633", Module1_0.05 = "#003d80", Module1_0.01 = "#003d80")
...
A5N.only$final_DES = gsub(paste(bad_name, collapse = "|"), "", A5N.only$new_des) #was originally A5N.only$ABACUS_DES
A5N.only$final_DES = str_wrap(A5N.only$final_DES,width = 20) #was originally A5N.only$ABACUS_DES
A5N.only$final_DES = gsub(",.*","",A5N.only$final_DES) #was originally A5N.only$ABACUS_DES
...
ggplot(df3,aes(x = LogFoldChange, y = log.p.value, cex = 2, fill = factor(group), color = factor(group))) + 
  ...
  geom_text_repel(data=A5N.only, #separate dataset for specific labels
                  aes(label= final_DES, size = 2), #call protein name, font size?
